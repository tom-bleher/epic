...
static Ref_t create_B0Tracker(Detector& description, xml_h e, SensitiveDetector sens) {
  ...
  // CAD support-plate components: built once, placed per station
  std::map<std::string, std::vector<SupportComponentDef>> supportComponents;
  for (const std::string supportName :
       {"B0SupportPlate1", "B0SupportPlate2", "B0SupportPlate3", "B0SupportPlate4"}) {
    xml_h supportModule = findModule(supportName);
    if (!supportModule.ptr()) {
      throw std::runtime_error("B0Tracker: <module name=\"" + supportName + "\"> not found");
    }
    ...
    if (!x_polygon.ptr()) {
      throw std::runtime_error("B0Tracker: " + supportName + " lacks <extruded_polygon>");
    }
