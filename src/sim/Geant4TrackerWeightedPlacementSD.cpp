...
namespace sim {
  ...
  /**
     ...
     *
     *  @{
     *  \package Geant4TrackerWeightedPlacementAction
     *
  ...
  /**
     *  Geant4SensitiveAction<TrackerWeightedPlacement>
     *
     *
     *  \author  M.Frank
     *  \version 1.0
     ...
     */
  struct TrackerWeightedPlacement {

    // This action intentionally preserves the DD4hep v01-35 weighted-hit
    // behavior. The only functional change is that accumulated state is keyed
    // by the complete Geant4 touchable placement rather than the reused leaf
    // physical-volume pointer.
    using PlacementElement = std::pair<const G4VPhysicalVolume*, int>;
    using PlacementKey     = std::vector<PlacementElement>;

    static PlacementKey placementKey(const G4StepPoint* point) {
      PlacementKey key;
      ...
      key.reserve(static_cast<std::size_t>(depth + 1));
      for (int level = 0; level <= depth; ++level) {
        key.emplace_back(touchable->GetVolume(level), touchable->GetCopyNumber(level));
      }
      return key;
    }
    ...
    /// Default constructor
    TrackerWeightedPlacement() = default;

    /// Clear collected information and restart for new hit
    TrackerWeightedPlacement& clear() {
    ...
    /// Start a new hit
    TrackerWeightedPlacement& start(const G4Step* step, const G4StepPoint* point) {
      if (DEBUG == printLevel()) {
        std::cout << " DEBUG: Geant4TrackerWeightedPlacementSD::start(const G4Step* step, const "
                     "G4StepPoint* point) ...."
                  << std::endl;
      ...
      thisPlacement = placementKey(point);

      return *this;
    }

    /// Update energy and track information during hit info accumulation
    TrackerWeightedPlacement& update(const G4Step* step) {
      if (DEBUG == printLevel()) {
        std::cout << " DEBUG: Geant4TrackerWeightedPlacementSD::update(const G4Step* step) ...."
                  << std::endl;
        Geant4StepHandler h(step);
        dumpStep(h, step);
      }
    ...
    TrackerWeightedPlacement& calc_dist_out(const G4VSolid* solid) {
    ...
    TrackerWeightedPlacement& calc_dist_in(const G4VSolid* solid) {
    ...
    void extractHit(Geant4HitCollection* collection, EInside ended) {
      if (DEBUG == printLevel()) {
        std::cout << " DEBUG: Geant4TrackerWeightedPlacementSD::extractHit(Geant4HitCollection* "
                     "collection, EInside ended) ...."
                  << std::endl;
      ...
      if (current != -1) {
        ...
        Geant4Tracker::Hit* hit = new Geant4Tracker::Hit(pre.truth.trackID, pre.truth.pdgID,
                                                         deposit, time, step_length, pos, mom);
        hit->flag               = hit_flag;
        hit->cellID             = cell;
        hit->g4ID               = g4ID;
        ...
        sensitive->print(
            ...
            pre.truth.trackID, int(collection->GetSize()), combined, pre.truth.deposit / CLHEP::keV,
            ...
            ((hit_flag & Geant4Tracker::Hit::HIT_STARTED_INSIDE) ? "INSIDE " : ""), dist_in,
            ((hit_flag & Geant4Tracker::Hit::HIT_ENDED_SURFACE) ? "SURFACE" : ""),
            ((hit_flag & Geant4Tracker::Hit::HIT_ENDED_OUTSIDE) ? "OUTSIDE" : ""),
            ((hit_flag & Geant4Tracker::Hit::HIT_ENDED_INSIDE) ? "INSIDE " : ""), dist_out);
        collection->add(hit);
      }
      clear();
    }
    ...
    G4bool process(const G4Step* step, G4TouchableHistory*) {
      Geant4StepHandler h(step);
      if (DEBUG == printLevel()) {
        std::cout << " DEBUG: Geant4TrackerWeightedPlacementSD::process(const G4Step* step, "
                     "G4TouchableHistory* ) ...."
                  << std::endl;
        dumpStep(h, step);
      }
      ...
      if (current == h.trkID() && thisPV != 0 && placementKey(h.pre) != thisPlacement) {
        if (DEBUG == printLevel()) {
          std::cout << " DEBUG: Geant4TrackerWeightedPlacementSD: if ( current == h.trkID() && "
                       "thisPV != 0 && placementKey(h.pre) != thisPlacement ),"
                    << " Track went into new Volume, extracted the hit in prePV, then start a new "
                       "hit in thisPV."
                    << std::endl;
        }
    ...
    void startEvent() { thisSD = dynamic_cast<G4VSensitiveDetector*>(&sensitive->detector()); }

    ///dumpStep
    void dumpStep(const Geant4StepHandler& h, const G4Step* s) {
      std::stringstream str;
      str << " ----- step in detector " << h.sdName(s->GetPreStepPoint()) << " prePos  "
          << h.prePos() << " postPos " << h.postPos() << " preStatus  " << h.preStepStatus()
          << " postStatus  " << h.postStepStatus() << " preVolume "
          << h.volName(s->GetPreStepPoint()) << " postVolume " << h.volName(s->GetPostStepPoint())
          << std::endl
  ...
  template <> void Geant4SensitiveAction<TrackerWeightedPlacement>::initialize() {
  ...
  template <>
  void Geant4SensitiveAction<TrackerWeightedPlacement>::begin(G4HCofThisEvent* /* hce */) {
    m_userData.startEvent();
  }

  /// G4VSensitiveDetector interface: Method invoked at the end of each event.
  template <>
  void Geant4SensitiveAction<TrackerWeightedPlacement>::end(G4HCofThisEvent* /* hce */) {
    m_userData.endEvent();
  }

  /// Define collections created by this sensitivie action object
  template <> void Geant4SensitiveAction<TrackerWeightedPlacement>::defineCollections() {
    m_collectionID = declareReadoutFilteredCollection<Geant4Tracker::Hit>();
  }

  /// Method for generating hit(s) using the information of G4Step object.
  template <>
  void Geant4SensitiveAction<TrackerWeightedPlacement>::clear(G4HCofThisEvent* /* hce */) {
    m_userData.clear();
  }

  /// Method for generating hit(s) using the information of G4Step object.
  template <>
  G4bool Geant4SensitiveAction<TrackerWeightedPlacement>::process(const G4Step* step,
  ...
  template <>
  bool
  Geant4SensitiveAction<TrackerWeightedPlacement>::processFastSim(const Geant4FastSimSpot* spot,
                                                                  G4TouchableHistory* history) {
    return m_userData.process(spot, history);
  }
  typedef Geant4SensitiveAction<TrackerWeightedPlacement> Geant4TrackerWeightedPlacementAction;
} // namespace sim
} // namespace dd4hep
...
DECLARE_GEANT4SENSITIVE(Geant4TrackerWeightedPlacementAction)
